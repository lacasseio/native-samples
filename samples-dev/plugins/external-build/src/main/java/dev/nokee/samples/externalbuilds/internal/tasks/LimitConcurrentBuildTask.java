...
//                    if (tokens.length != 2) throw new RuntimeException("assumption violated");
                    ...
                    final Provider<ExternalBuildConcurrencyService> maxGradleBuild = project.getGradle().getSharedServices().registerIfAbsent(String.valueOf(((ExternalBuildTask) task).getProjectDirectory().get().getAsFile().hashCode()), ExternalBuildConcurrencyService.class, spec -> {
